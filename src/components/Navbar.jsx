...
import "./shimmer.css"; // Assuming you keep this for other shimmer effects
    ...
    <header className="fixed top-0 inset-x-0 z-50">
      ...
      >
        <div className="flex justify-between items-center h-16 font-cohere">
                ...
                >
                  <Link
                    ...
                    className={`text-lg transition px-2 py-1 rounded-md
                      ${
                        active
                          ? // --- UPDATED ACTIVE CLASS HERE ---
                            "font-semibold text-black active-gradient-border"
                          : // --- UPDATED HOVER CLASS HERE ---
          ...
          {/* RIGHT — CTA BUTTON (NEON SHADOW FROM IMAGE) */}
        <Link
  to="/request-call"
className="
...
>
  <span
    className="
      absolute inset-0 -z-10 rounded-full blur-xl opacity-80
  ...
  Request a Call
          ...
          {/* MOBILE MENU ICON */}
          <button className="lg:hidden text-black text-3xl">≡</button>
        </div>
      </motion.nav>
    </header>
  );
